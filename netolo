package org.skobelev;

import org.telegram.telegrambots.bots.TelegramLongPollingBot;
import org.telegram.telegrambots.meta.TelegramBotsApi;
import org.telegram.telegrambots.meta.api.methods.send.SendMessange;
...
public class Main {
    ...
    private static void ConcurrentHashMap<PomodoroBot.Timer, Long> userTimers = new ConcurrentHashMap();
        ...
        public void onUpdateReceived(Update update) {
            if (update.hasMessage() && update.getMessage().hasText()) {
                ...
                if (update.getMessage().getText().equals("/start")) {
                    sendMsg("""
                           Pomodoro - сделать свое время более эффективным.
        ...
        public void checkTimer() throws InterruptedException {
            while(true) {
                ...
                userTimers.forEach((timer, userId) -> {
                    System.out.println("Проверка userId = %d, server_time = %s, user_timer = %s\n",
                    ...
                    if (Instant.now().isAfter(timer.time)) {
                        ...
                        switch (timer.timerType) {
                            ...
                            case BREAK -> sendMsg("Таймер завершил свой работу", userId.toString());
                        }
                    }
                });
                Thread.sleep(1000);
